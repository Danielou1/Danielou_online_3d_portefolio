  ...
  server.post('/api/chat', async (req, res) => {
    ...
    try {
      ...
      if (botMessage === undefined) {
        return res.status(500).send({ error: 'Failed to get text from AI response.' });
      }
      ...
      return res.status(500).send({ error: 'Failed to get a response from the AI.' });
    }
  });
